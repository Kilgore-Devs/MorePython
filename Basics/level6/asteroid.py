...
def asteroid_collision(asteroids):
    ...
    for asteroid in asteroids:
        while rocks and asteroid < 0 and rocks[-1] > 0:
            # While there are asteroids moving in opposite directions and the current asteroid is moving to the left (negative)
            if abs(asteroid) > rocks[-1]:
                # If the current asteroid is larger, pop the smaller asteroid
                rocks.pop()
            elif abs(asteroid) == rocks[-1]:
                # If they are of equal size, both asteroids explode
                rocks.pop()
                break
            else:
                # If the current asteroid is smaller, it explods, and  continue checking with the next asteroid in the list
                break
        else:
            # If no collision occurred, add the current asteroid to the list
            rocks.append(asteroid)
    return rocks

print(asteroid_collision([]))# [])
